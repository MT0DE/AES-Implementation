...
int main(int argc, char const *argv[])
{
    ...
    //Stream in 16 bytes at a time and encrypt each block separatley
    int totalBytes= 0;
    short flag = 1;
    int blocksCounter = 0;
    int amountWriteBack = 0;
    counter = 1;
    while((counter = read(STDIN_FILENO, blocks, maxBytes)) != 0){
...
unsigned char gfMulti(unsigned char byte, int term){
    ...
    unsigned char shifted = xtime(byte);
    if (term == 3)
    {
        return shifted ^ copy;
    }
...
void addRoundKey(unsigned char *block, int blockSize){
    ...
    unsigned char roundkey_index = 0;
    for (int i = 0; i < 4; i++)
    {
